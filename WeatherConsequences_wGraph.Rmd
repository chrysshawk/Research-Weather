...
Specifically, it seeks to answer two questions: (1) Across the United States, which types of events are most harmful with respect to population health, and (2) Across the United States, which types of events have the greatest economic consequences?  
...
Economic consequences have been categorized as both property expenses and crop expenses. The table below (Figure 3) summarizes which weather events have the greatest economic consequences, and also gives insight into how many of the weather events have been recorded, as well as the mean expense per occurrence.
```{r econResults}
...
If the goal is to minimize the economic impact of weather events, we would advice to have a plan for mitigating the effects of floods in general. However, given the significant effect of hurricanes/tornados when they first strike, one should also be prepared for these events.

```{r econGraph}
# Grouping top10 events for graph, converting to million USD
econSummary <- econEvents %>%
     select(EVTYPE, DamageType, fullExpense) %>%
     filter(EVTYPE %in% top10Econ$EVTYPE) %>%
     group_by(EVTYPE, DamageType) %>%
     summarize(Events = length(EVTYPE),
               Expenses = round(sum(fullExpense)/1000000,0),
               MeanExpense = mean(fullExpense)/1000000) %>%
     arrange(desc(Expenses))
...
ggplot(data = head(econSummary, 10), 
     ...
     geom_text(aes(label = Expenses), size = 4) +
     theme_light() +
     labs(title = "Weather events with highest economic expense in the US", 
          x = "Weather event", y = "Total expenses (Million USD)")
```


## Recommendation

In this analysis we have investigated the impact weather events in the US have had on population health and economic expenses. 

Tornados have had both significant effects on population health (fatalities and injuries) as well as economic expenses. On the other hand, floods by far have the overall greatest economic expense.

To minimize the impact of weather events on population health and economic expenses, we advice to have a preparedness plan for these events, including a contingency plan for when they occur.
